  ...
  `).join('') :
        ""}
          ...
          <div class="total-amount">
            ...
            <div><strong>${(Number(subTotal) + Number(allTaxTotal)).toFixed(2)}</strong></div>
          </div>
        </div>
      </div>
      <!--Footer -->
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-white px-4 py-4 shadow-sm">
        <View className="flex-row justify-between items-center">
          <TouchableOpacity className="p-2 rounded-full bg-gray-100" onPress={() => navigation.navigate('Purchase')}>
            <ArrowLeft stroke="#333" width={22} height={22} />
          </TouchableOpacity>
          <Text className="text-xl font-bold text-gray-800">Purchase Invoice</Text>
          ...
          <View className="m-4 bg-white rounded-lg shadow-sm overflow-hidden">
            ...
            <View className="flex-row p-1">
              ...
              <View className="w-1/2 p-2 border border-gray-200">
                ...
                <Text>{invoices.vendor?.displayName}</Text>
                <Text className="text-gray-600 text-sm">{invoices.vendor?.billingAddress?.addressLine1}</Text>
                <Text className="text-gray-600 text-sm">
                  State : {invoices.vendor?.billingAddress?.state || ''}
                </Text>
                <Text className="text-gray-600 text-sm">Ph: {invoices.vendor?.billingAddress?.phone || "--"} </Text>
                <Text className="text-gray-600 text-sm">{invoices.vendor?.gstin || ''}</Text>
              </View>
              ...
              <View className="w-1/2 ">
                <View className="flex-row  border border-gray-200 p-2">
                  ...
                  <Text className="w-1/2 text-[13px]">{invoices.purchaseOrderNumber}</Text>
                </View>
                <View className="flex-row  border border-gray-200 p-2">
                  <Text className="w-1/2 font-bold text-[13px]">Invoice Date:</Text>
                  <Text className="w-1/2 text-[13px]">{invoices.purchaseDate}</Text>
                </View>
                <View className="flex-row  border border-gray-200 p-2">
                  <Text className="w-1/2 font-bold text-[13px]">Ship To:</Text>
                  <Text className="w-1/2 text-sm text-[13px]">{invoices.vendor?.shippingAddress.addressLine1 || "--"}</Text>
                </View>
                <View className="flex-row  border border-gray-200 p-2">
                  <Text className="w-1/2 font-bold text-[13px]">Supplier Location:</Text>
                  <Text className="w-1/2 text-[13px]">{invoices.vendor?.placeOfSupply}</Text>
                </View>
                <View className="flex-row  border border-gray-200 p-2 ">
            ...
            <ScrollView horizontal>
              <View className="min-w-full">
                ...
                {invoices.items?.map((item, index) => {
                  ...
                  let totalAmount = Number(item.purchasePrice) * Number(item.quantity)
